...
function display_problem(){
	...
	if($_SESSION["status"]=="Admin") $data = mysqli_query($link,"SELECT * FROM problems WHERE pid=$pid ");
	else $data = mysqli_query($link,"SELECT * FROM problems WHERE status='Active' and pid=$pid");
	if($pid!=0) if(!($data instanceof mysqli_result) || mysqli_num_rows($data)!=1){
	...
	if($pid==0){
		...
		if(($g=mysql_getdata("SELECT distinct pgroup FROM problems WHERE status='Active' ORDER BY pgroup"))!=NULL){
			...
			foreach($g as $i=>$gn) echo "<option value='group".($i+1)."'>".preg_replace("/^#[0-9]+ /i","",($gn==""?"Unclassified":$gn))."</option>";
			...
			foreach($g as $i=>$gn){
				...
				while($problem = mysqli_fetch_array($data)){
					...
					if(($t instanceof mysqli_result) && mysqli_num_rows($t) && $t=mysqli_fetch_array($t)) $statistics = "<a title='Accepted Solutions / Total Submissions' href='?display=submissions&pid=$problem[pid]'>".$t["ac"]." / ".$t["tot"]."</a>"; else $statistics = "NA";
	...
	if(($t instanceof mysqli_result) && mysqli_num_rows($t) && $t=mysqli_fetch_array($t)) $statistics = "<a title='Accepted Solutions / Total Submissions' href='?display=submissions&pid=$pid'>".$t["ac"]."/".$t["tot"]."</a>"; else $statistics = "NA";
	...
	if(($data instanceof mysqli_result) && mysqli_num_rows($data)>0) if(mysqli_num_rows($data)){
		...
		while($temp = mysqli_fetch_array($data)){
			$teamname = mysqli_query($link,"SELECT teamname FROM teams WHERE tid=".$temp["tid"]); if(($teamname instanceof mysqli_result) && mysqli_num_rows($teamname)==1){ $teamname = mysqli_fetch_array($teamname); $teamname=$teamname["teamname"]; } else $teamname="Anonymous";
	...
	if($_SESSION["tid"]==0) echo "<center>Please login to submit solutions.</center>";
	...
	else if($admin["mode"]=="Passive" && $_SESSION["status"]!="Admin" && preg_match("/^CQM\-[0-9]+$/i",$pgroup)) echo "<center>You can no longer submit solutions to this problem.</center>";
	else {
		...
		if(isset($_GET["edit"])){
			...
			if(($t instanceof mysqli_result) && mysqli_num_rows($t)==1){
				...
				if($_SESSION["tid"]==$run["tid"] || $run["access"]=="public" || $_SESSION["status"]=="Admin") $editcode=preg_replace("<","&lt;",$run["code"]);
...
function action_makeproblem(){
	...
	foreach($_POST as $key=>$value) if(preg_match("^make_",$key)) if(empty($_POST[$key]) && $key!="make_type" && $key!="make_score" && $key!="make_options") { $_SESSION["message"][] = "Problem Creation Error : Insufficient (Text) Data".$key; return; }
	...
	foreach($_POST as $key=>$value) if(preg_match("^make_",$key) && preg_match($invalidchars,$value) ){ $_SESSION["message"][] = "Problem Creation Error : Value of $key contains invalid characters."; return; }
	foreach($_POST as $key=>$value) if(preg_match("^make_",$key) && $key!="make_languages" && strlen($value)>30 ){ $_SESSION["message"][] = "Problem Creation Error : Value of $key too long."; return; }
	if(empty($_POST["make_score"])) $_POST["make_score"] = "0";
	$temp1 = $temp2 = array(); foreach($_POST as $key=>$value) if(preg_match("^make_",$key) && !preg_match("^make_file_",$key)){ $temp1[]=preg_replace("^make_","",$key); $temp2[]=filter($value); }
	foreach(array("statement","input","output") as $item){
		...
		$temp1[]=$item; $temp2[]=addslashes(preg_replace("\r","",file_get("sys/temp/".$sessionid."_$item.$ext"))); unlink("sys/temp/".$sessionid."_$item.$ext");
		}
...
function action_updateproblem(){
	...
	foreach($_POST as $key=>$value) if(preg_match("/^update_/i",$key) && preg_match($invalidchars,$value) ){ $_SESSION["message"][] = "Problem Updation Error : Value of $key contains invalid characters."; return; }
	foreach($_POST as $key=>$value) if(preg_match("/^update_/i",$key) && $key!="update_languages" && strlen($value)>30 && $key!="update_languages"){ $_SESSION["message"][] = "Problem Updation Error : Value of $key too long."; return; }

	$pid = $_POST["update_pid"];
	foreach($_POST as $key=>$value) if(preg_match("/^update_/i",$key) && !preg_match("^update_file_",$key) && $key!="update_pid" && $key!="update_delete"){
		mysqli_query($link,"UPDATE problems SET ".preg_replace("/^update_/i","",$key)."='".addslashes(preg_replace("\"","\'",$value))."' WHERE pid=$pid");
		}
	...
	if(0)if(isset($_POST["update_status"]) && $_POST["update_status"]=="Delete") mysqli_query($link,"DELETE FROM problems WHERE pid=$pid");
...
function action_makeproblemactive(){
	global $link;
	if($_SESSION["status"]!="Admin"){ $_SESSION["message"][] = "Access Denied : You need to be an Administrator to perform this action."; return; }
	if(isset($_GET["pid"]) && !empty($_GET["pid"]) && is_numeric($_GET["pid"])) mysqli_query($link,"UPDATE problems SET status='Active' WHERE pid=".$_GET["pid"]);
...
function action_makeprobleminactive(){
	global $link;
	if($_SESSION["status"]!="Admin"){ $_SESSION["message"][] = "Access Denied : You need to be an Administrator to perform this action."; return; }
	if(isset($_GET["pid"]) && !empty($_GET["pid"]) && is_numeric($_GET["pid"])) mysqli_query($link,"UPDATE problems SET status='Inactive' WHERE pid=".$_GET["pid"]);
	else { $_SESSION["message"][] = "Problem Status Updation Error : Insufficient Data"; return; }
	}
