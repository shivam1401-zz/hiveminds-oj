...
function action_adminwork(){
	global $admin,$fullresult,$link;
	// Update Team Scores
	$data = mysqli_query($link,"SELECT pid,score FROM problems WHERE status='Active'"); $score = array();
	...
	if(($data instanceof mysqli_result)) while($temp = mysqli_fetch_array($data)){ $tid = $temp["tid"];
		...
		if(($prob instanceof mysqli_result)){ $solvedn = mysqli_num_rows($prob);
			while($temp = mysqli_fetch_array($prob)){ $pid = $temp["pid"]; $score+=$temp["score"];
				...
				if(is_resource($sub) && $t = mysqli_fetch_array($sub)){ $penalty+=$t["submittime"];
					...
					if(($sub instanceof mysqli_result) && $t = mysqli_fetch_array($sub)) $penalty+=$t["incorrect"]*(isset($admin["penalty"])?($admin["penalty"]*60):0);
					}
				}
			}
	...
	if(($data instanceof mysqli_result)) for($rank=1; $temp = mysqli_fetch_array($data); $rank++){
		$solvedn = mysqli_query($link,"SELECT count(distinct(runs.pid)) as n FROM runs,problems WHERE runs.tid='$temp[tid]' and runs.result='AC' and runs.pid=problems.pid and problems.status='Active'");
		if(($solvedn instanceof mysqli_result) && mysqli_num_rows($solvedn)==1){ $solvedn = mysqli_fetch_array($solvedn); $solvedn=$solvedn["n"]; } else $solvedn=0;
	...
	if(($g=mysql_getdata("SELECT distinct pgroup FROM problems WHERE status='Active' ORDER BY pgroup"))!=NULL){
		...
		foreach($g as $gn){
			$json.= "<tr><th colspan=2>".preg_replace("/^#[0-9]+ /i","",($gn==""?"Unclassified":$gn))."</th></tr><tr class='AC'><td><i>Problem Name</i></td><td><i>Points</i></td></tr>";
...
function action_ajaxrefresh($type){
	global $admin,$fullresult,$ajaxlogout,$link;
	...
	if($_SESSION["tid"]==0){
		...
		if(is_resource($data)) while($temp = mysqli_fetch_array($data)){
			if($_SESSION["status"]=="Admin") $t = mysqli_query("SELECT name,code FROM problems WHERE pid=$temp[pid]");
			else $t = mysqli_query("SELECT name,code FROM problems WHERE pid=$temp[pid] and status='Active'");
			if(is_resource($t) && mysql_num_rows($t)==1){ $t = mysqli_fetch_array($t); $probname=$t['name']; $probcode=$t['code']; } else continue;
			$result = $temp["result"]; if(isset($fullresult[$result])) $result = $fullresult[$result];
	...
	if($_SESSION["tid"]!=0){
		...
		if(($t instanceof mysqli_result)) $t = mysqli_fetch_array($t);
		$solvedn = mysqli_query($link,"SELECT count(distinct(runs.pid)) as n FROM runs,problems WHERE runs.tid='$tid' and runs.result='AC' and runs.pid=problems.pid and problems.status='Active'");
		if(($solvedn instanceof mysqli_result) && mysqli_num_rows($solvedn)==1){ $solvedn = mysqli_fetch_array($solvedn); $solvedn=$solvedn["n"]; } else $solvedn=0;
	...
	if($_SESSION["tid"]!=0){
		...
		if(($temp instanceof mysqli_result) && mysqli_num_rows($temp)==1){
	...
	if(($data instanceof mysqli_result) && mysqli_num_rows($data)){
		$json["newclar"]=array();
		while($temp = mysqli_fetch_array($data)){
			if($temp["pid"]==0) $prob = array("name"=>"General"); else $prob = mysqli_fetch_array(mysqli_query($link,"SELECT name FROM problems WHERE pid=".$temp["pid"]));
