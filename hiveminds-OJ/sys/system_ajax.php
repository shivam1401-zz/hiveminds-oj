...
function action_adminwork()
{
    ...
    if (($data instanceof mysqli_result))
        while ($temp = mysqli_fetch_array($data)) {
            ...
            if (($prob instanceof mysqli_result)) {
                ...
                while ($temp = mysqli_fetch_array($prob)) {
                    ...
                    if (is_resource($sub) && $t = mysqli_fetch_array($sub)) {
                        ...
                        if (($sub instanceof mysqli_result) && $t = mysqli_fetch_array($sub))
                            $penalty += $t["incorrect"] * (isset($admin["penalty"]) ? ($admin["penalty"] * 60) : 0);
                    }
                }
            }
            if ($penalty == 0)
                $penalty = 2000000000; // just to put someone with at least one AC submission (of 0 points) above others without it
    ...
    if (($data instanceof mysqli_result))
        for ($rank = 1; $temp = mysqli_fetch_array($data); $rank++) {
            ...
            if (($solvedn instanceof mysqli_result) && mysqli_num_rows($solvedn) == 1) {
        ...
        if (is_resource($data))
            while ($temp = mysqli_fetch_array($data)) {
                if ($_SESSION["status"] == "Admin")
                    $t = mysqli_query("SELECT name,code FROM problems WHERE pid=$temp[pid]");
                else
                    $t = mysqli_query("SELECT name,code FROM problems WHERE pid=$temp[pid] and status='Active'");
