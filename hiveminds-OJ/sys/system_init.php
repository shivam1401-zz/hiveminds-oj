...
while($e=readdir($f)){
	if($e=="."||$e=="..") continue;
	if(preg_match("/^system/i",$e) && file_exists("sys/$e") && $e!="system_init.php" && $e!="system_config.php")
		include("sys/$e");
	}
closedir($f);
...
session_start();
$sessionid = (SID=="")?$_REQUEST["PHPSESSID"]:preg_replace("\PHPSESSID=\i","",SID);
...
if($_SESSION["status"]!="Admin" && (!isset($_SESSION["ghost"])||!$_SESSION["ghost"]) && (!isset($_GET["action"])||$_GET["action"]!="ajaxrefresh")) mysqli_query($link,"INSERT INTO logs VALUES (".(time()).",'".$_SERVER["REMOTE_ADDR"]."','".$_SESSION["tid"]."','".addslashes(json_encode($_GET))."')");
...
if(isset($_GET["download"]) and in_array($_GET["download"],array("code","output")) and isset($_GET["rid"]) and is_numeric($_GET["rid"])){
	if($_SESSION["status"]!="Admin" and $_GET["download"]=="output"){
		echo "Indeed Hive Mind : You do not have the authorization to download this resource.";
		exit;
		}
	if($_SESSION["status"]=="Admin") $t = mysqli_query("SELECT * FROM runs WHERE rid=".$_GET["rid"]." and access!='deleted'");
	else $t = mysqli_query($link,"SELECT * FROM runs WHERE rid=".$_GET["rid"]." and ((access='private' and tid=".$_SESSION["tid"].") or access='public')");
	if(($t instanceof mysqli_result) && mysql_num_rows($t)){
		...
		header("Content-Disposition: attachment; filename= \"Indeed Hive Mind - Run ID $_GET[rid] - ".ucwords($_GET["download"]).$ext."\"");
		header("Content-Length: ".strlen($t[$_GET["download"]]));
		header("Content-Transfer-Encoding: binary"); 
		echo $t[$_GET["download"]];
		}
	else echo "Indeed Hive Mind : The requested ".$_GET["download"]." could not be found in the Database.";
	exit;
	}
...
if(isset($_GET["download"]) and in_array($_GET["download"],array("statement","input","output")) and isset($_GET["pid"]) and is_numeric($_GET["pid"])){
	...
	$t = mysqli_query($link,"SELECT * FROM problems WHERE pid=$_GET[pid] and status!='Delete'");
	if(($t instanceof mysqli_result) && mysql_num_rows($t)){
		...
		//header("Cache-Control: private",false);
		header("Content-Type: application/force-download");
		header("Content-Disposition: attachment; filename= \"Indeed Hive Mind - Problem ID $_GET[pid] - ".ucwords($_GET["download"]).".txt\"");
		header("Content-Length: ".strlen($t[$_GET["download"]]));
		header("Content-Transfer-Encoding: binary"); 
		echo $t[$_GET["download"]];
		}
	else echo "Indeed Hive Mind : The requested problem could not be found in the Database.";
	exit;
	}
...
if(isset($_GET["image"]) && is_numeric($_GET["image"])){
	$pid = $_GET["image"];
	if($_SESSION["status"]=="Admin") $t = mysqli_query("SELECT * FROM problems WHERE pid='$pid'");
	else $t = mysqli_query($link,"SELECT * FROM problems WHERE pid='$pid' and status='Active'");
	$t["image"] = "";
	if(($t instanceof mysqli_result) && mysqli_num_rows($t)==1) $t = mysqli_fetch_array($t);
	if(!empty($t["image"])){
	...
	else echo "Indeed Hive Mind : The requested image could not be found in the Database.";
	exit;
	}
...
if(isset($_GET["sql"])){
	...
	$data = array();
	if(is_resource($result)) while($row = mysqli_fetch_array($result)) $data[] = $row;
