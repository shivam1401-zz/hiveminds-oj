...
if (isset($_GET["download"]) and in_array($_GET["download"], array(
    "code",
    "output"
)) and isset($_GET["rid"]) and is_numeric($_GET["rid"])) {
    ...
    if ($_SESSION["status"] == "Admin")
        $t = mysqli_query("SELECT * FROM runs WHERE rid=" . $_GET["rid"] . " and access!='deleted'");
    else
        $t = mysqli_query($link, "SELECT * FROM runs WHERE rid=" . $_GET["rid"] . " and ((access='private' and tid=" . $_SESSION["tid"] . ") or access='public')");
    if (($t instanceof mysqli_result) && mysql_num_rows($t)) {
        ...
        echo "Indeed Hive Mind : The requested " . $_GET["download"] . " could not be found in the Database.";
    exit;
}
...
if (isset($_GET["download"]) and in_array($_GET["download"], array(
    ...
    "input",
    "output"
)) and isset($_GET["pid"]) and is_numeric($_GET["pid"])) {
    if ($_SESSION["status"] != "Admin") {
        echo "Indeed Hive Mind : You do not have the authorization to download this resource.";
        exit;
    }
    ...
    if (($t instanceof mysqli_result) && mysql_num_rows($t)) {
        ...
        echo "Indeed Hive Mind : The requested problem could not be found in the Database.";
    exit;
}
...
if (isset($_GET["image"]) && is_numeric($_GET["image"])) {
    $pid = $_GET["image"];
    if ($_SESSION["status"] == "Admin")
        $t = mysqli_query("SELECT * FROM problems WHERE pid='$pid'");
    else
        $t = mysqli_query($link, "SELECT * FROM problems WHERE pid='$pid' and status='Active'");
    $t["image"] = "";
    ...
    if (!empty($t["image"])) {
        ...
        echo "Indeed Hive Mind : The requested image could not be found in the Database.";
    exit;
}
...
if (isset($_GET["action"])) {
    ...
    if ($_GET["action"] == "problem-status" and ($_GET["type"] == "Active" or $_GET["type"] == "Inactive"))
    ...
    if ($_GET["action"] == "group-add")
        action_group_add(); // to team
    if ($_GET["action"] == "group-remove")
        action_group_remove(); // to team
