...
function display_submissions(){
	global $admin,$fullresult,$currentmessage,$extension,$link;
	...
	if(isset($_GET["tid"]) && !empty($_GET["tid"]) && is_numeric($_GET["tid"])){
		...
		if(($t instanceof mysqli_result) && mysqli_num_rows($t)){
			$filter["tid"] = $_GET["tid"];
	...
	if(isset($_GET["pgr"]) && !empty($_GET["pgr"]) && in_array($_GET["pgr"],$g)){
		...
		if(($t instanceof mysqli_result) && mysqli_num_rows($t)){
			...
			$probdata = mysqli_fetch_array($t);
			$filters[]="<a href='?".str_replace("&pgr=".urlencode($filter["pgroup"]),"",$urlargs)."'>".filter(preg_replace("/^#[0-9]+ /i","",$filter["pgroup"]))."</a>";
	...
	if(isset($filter["tid"])){
		...
		if(($data instanceof mysqli_result)){
			$solvedn = mysqli_num_rows($data); $solvedp = array();
			while($temp = mysqli_fetch_array($data)) $solvedp[]="<a href='?display=problem&pid=$temp[pid]' title=\"$temp[code]\">$temp[name]</a>";
	...
	if(!isset($filter["result"])){
		...
		if(($t1 instanceof mysqli_result)) for($info2=array();$t2=mysqli_fetch_array($t1);){ if($t2["result"]=="") $t2["result"]="..."; $info2[$t2["result"]]=$t2["cnt"]; } if(!isset($info2["..."])) $info2["..."]=0;
	...
	if(!isset($filter["language"])){
		...
		if(($t1 instanceof mysqli_result)) for($info2=array();$t2=mysqli_fetch_array($t1);) $info2[$t2["language"]]=$t2["cnt"];
	...
	$total = mysqli_query($link,"SELECT count(*) as total FROM runs WHERE access!='deleted' AND tid in (SELECT tid FROM teams WHERE status='Normal' OR status='Admin') AND pid in (SELECT pid FROM problems WHERE status".(($_SESSION["status"]=="Admin")?"!='Delete'":"='Active'").") $condition ORDER BY rid DESC");
	...
	if(($data instanceof mysqli_result)) $n = mysqli_num_rows($data);
	if(($data instanceof mysqli_result)) for($i=0;$temp = mysqli_fetch_array($data);$i++){
		if($i==$perpage) break;
		if($temp["language"]=="Brain") $temp["lan"]="Brainf**k"; else $temp["lan"]=$temp["language"];
		$t = mysqli_query($link,"SELECT teamname FROM teams WHERE tid=$temp[tid] and (status='Normal' or status='Admin')"); if(($t instanceof mysqli_result) && mysqli_num_rows($t)==1){ $t = mysqli_fetch_array($t); $teamname=$t['teamname']; } else continue;
		$t = mysqli_query($link,"SELECT name FROM problems WHERE pid=$temp[pid] and status".(($_SESSION["status"]=="Admin")?"!='Delete'":"='Active'").";"); if(($t instanceof mysqli_result) && mysqli_num_rows($t)==1){ $t = mysqli_fetch_array($t); $probname=$t['name']; } else continue;
...
function display_rankings(){
	global $admin,$currentmessage,$link;
	...
	$total = mysqli_query($link,"SELECT count(*) as total FROM teams WHERE status='Normal'".($group==0?"":" AND gid=$group;"));
	$total = mysqli_fetch_array($total); $total = $total["total"];
	...
	if(($data instanceof mysqli_result)){
		...
		while($row=mysqli_fetch_assoc($data)) echo "<option value=$row[gid] ".($row["gid"]==$group?"selected='selected'":"").">$row[groupname]</option>";
		echo "</select><br><br>";
		}
	...
	if(($data instanceof mysqli_result)) for($rank=($page-1)*$perpage+1; $temp = mysqli_fetch_array($data); $rank++){
		$solvedn = mysqli_query($link,"SELECT count(distinct runs.pid) as x FROM runs,problems WHERE runs.tid='$temp[tid]' and runs.result='AC' and runs.pid=problems.pid and problems.status='Active'");
		$solvedn = mysqli_fetch_array($solvedn); $solvedn = $solvedn["x"];
		$solveda = mysqli_query($link,"SELECT count(distinct runs.pid) as x FROM runs,problems WHERE runs.tid='$temp[tid]' and runs.pid=problems.pid and problems.status='Active'");
		$solveda = mysqli_fetch_array($solveda); $solveda = $solveda["x"];
		
		$groupname = mysqli_query($link,"SELECT groupname FROM groups WHERE statusx<3 AND gid=$temp[gid];");
...
function display_scoreboard(){
	...
	$temp = mysqli_query($link,"SELECT value FROM admin WHERE variable='scoreboard'");
	if(($temp instanceof mysqli_result)&&mysqli_num_rows($temp)==1){ $temp = mysqli_fetch_array($temp); echo $temp["value"]; }
