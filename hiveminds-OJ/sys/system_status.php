...
function display_submissions()
{
    ...
    if (!isset($filter["result"])) {
        ...
        if (($t1 instanceof mysqli_result))
...
function display_rankings()
{
    ...
    $total = mysqli_fetch_array($total);
    $total = $total["total"];
    if (isset($admin["rankpage"]))
        $perpage = $admin["rankpage"];
    else
        $perpage = 25;
    $x       = paginate("display=rankings&group=$group", $total, $perpage);
    $page    = $x[0];
    $pagenav = $x[1];
    ...
    if (($data instanceof mysqli_result))
        for ($rank = ($page - 1) * $perpage + 1; $temp = mysqli_fetch_array($data); $rank++) {
            ...
            $solvedn = mysqli_fetch_array($solvedn);
            $solvedn = $solvedn["x"];
            ...
            $solveda = mysqli_fetch_array($solveda);
            $solveda = $solveda["x"];
            
            $groupname = mysqli_query($link, "SELECT groupname FROM groups WHERE statusx<3 AND gid=$temp[gid];");
            if (!($groupname instanceof mysqli_result) or mysqli_num_rows($groupname) == 0)
                $groupname = "";
            else {
