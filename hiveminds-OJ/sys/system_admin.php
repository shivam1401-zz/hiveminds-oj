...
function display_adminlogs(){
	...
	if($_SESSION["status"]=="Admin"){
		...
		while($team = mysqli_fetch_array($teams)) $teamnames[$team["tid"]] = $team["teamname"];
		$logs = mysqli_query($link,"SELECT * FROM logs ORDER BY time DESC LIMIT ".(($page-1)*$limit).",".$limit);
		if(($logs instanceof mysqli_result)) while($log = mysqli_fetch_array($logs)){
...
function display_executionprotocol(){
	...
	if(!$invalid){
		...
		if(!($temp instanceof mysqli_result) || mysqli_num_rows($temp)==0) $invalid=1;
		}
	if(!$invalid){
	...
	if(!$invalid){
		...
		if(($temp instanceof mysqli_result) && mysqli_num_rows($temp)>0){ $temp = mysqli_fetch_array($temp); $teamname = $temp["teamname"]; }
		else $teamname="NA";
	...
	if(file_exists("env/error.txt") && ($errormessage = file_get("env/error.txt"))!=""){
		...
		else if($run["language"]=="Perl" || $run["language"]=="PHP") $errormessage = preg_replace("/ [^ ]*/i".addslashes($filename)." "," $realname ",$errormessage);
		else if($run["language"]=="Python") $errormessage = preg_replace("/\"[^ ]*/i".addslashes($filename)."\"","\"$realname\"",$errormessage);
