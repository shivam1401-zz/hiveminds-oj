...
function display_code()
{
    ...
    if (!isset($_GET["rid"]) || empty($_GET["rid"]))
        ...
        $rid = $_GET["rid"];
    $run   = mysqli_query("SELECT * FROM runs WHERE rid=$rid AND access!='deleted'");
    $error = 0;
    if (!$error)
        if (!is_resource($run) || mysql_num_rows($run) == 0)
            $error = 1;
        else
            $run = mysql_fetch_array($run);
    if (!$error) {
        $problem = mysqli_query("SELECT * FROM problems WHERE pid=$run[pid]");
        if (!is_resource($problem) || mysql_num_rows($problem) == 0)
            $error = 2;
        else
            $problem = mysql_fetch_array($problem);
    }
    if (!$error) {
        $team = mysqli_query("SELECT * FROM teams WHERE tid=$run[tid]");
        if (!is_resource($team) || mysql_num_rows($team) == 0)
            $error = 3;
        else
    ...
    if ($error == 1)
        echo "<tr><td colspan=10 style='padding:30;'>Code you requested does not exist in the Database.</td></tr>";
    if ($error == 2)
        echo "<tr><td colspan=10 style='padding:30;'>The problem for which this code is a solution does not exist.</td></tr>";
    if ($error == 3)
        echo "<tr><td colspan=10 style='padding:30;'>The team which submitted this code does not exist.</td></tr>";
    if ($error == 4)
        echo "<tr><td colspan=10 style='padding:30;'>You are not authorized to access this code.</td></tr>";
    else if (!$error) {
        $filename = $run["name"] . "." . $extension[$run["language"]];
        $result   = $run["result"];
        if (isset($fullresult[$result]))
            $result = $fullresult[$result];
        $code = preg_replace("<", "&lt;", $run["code"]); //$code = filter($run["code"]); $code = preg_replace("\n","<br>",$code); $code = preg_replace("	","    ",$code); $code = preg_replace(" ","&nbsp;",$code);
        ...
        if (($run["result"] != "RTE" || $_SESSION["status"] == "Admin") && !empty($run["error"]))
            echo "<tr><th colspan=10>Error Message</th></tr><tr><td colspan=10 style='text-align:left;padding:0;'><div class='limit'><pre class='brush:text'>" . preg_replace("<br>", "\n", filter($run["error"])) . "</pre></div></td></tr>";
        if (($_SESSION["status"] == "Admin" || $run["access"] == "public") and isset($_GET["io"]) and $_GET["io"] == "yes") {
            ...
            for ($i = 0, $j = 0; $i < $k; $i++) {
                ...
                if (!$p and $problem['output'][$i] == $run['output'][$i])
                    continue;
            ...
            echo "</table>";
        } else if (($_SESSION["status"] == "Admin" || $run["access"] == "public") and in_array($run["result"], array(
                "AC",
                "WA",
                "PE",
                "RTE"
...
function action_submitcode()
{
    ...
    $data = mysqli_query("SELECT status,languages,pgroup FROM problems WHERE pid='$_POST[code_pid]'");
    ...
    if ($_SESSION["status"] != "Admin" && preg_match("^#[0-9]+ CQM\-[0-9]+$", $data["prgoup"]) && $admin["mode"] == "Passive") {
    ...
    if ($ext != -1) {
        ...
        $_POST["code_name"] = preg_replace(".(b|c|cpp|java|pl|php|py|rb|txt)$", "", basename($_FILES['code_file']['name']));
    }
    ...
    if (!empty($sourcecode)) {
        mysqli_query("INSERT INTO runs (pid,tid,language,name,code,access,submittime) VALUES ('$_POST[code_pid]','$_SESSION[tid]','$_POST[code_lang]','$_POST[code_name]','$sourcecode','private'," . time() . ")");
        $rid                   = mysql_insert_id();
...
function action_rejudge()
{
    ...
    mysqli_query("UPDATE runs SET time=NULL,result=NULL WHERE access!='deleted' $condition");
}
...
function action_makecodepublic()
{
    ...
    if (isset($_GET["rid"]) && !empty($_GET["rid"]) && is_numeric($_GET["rid"]))
        mysqli_query("UPDATE runs SET access='public' WHERE rid=" . $_GET["rid"]);
    else {
        $_SESSION["message"][] = "Run Data Updation Error : Insufficient Data";
        return;
    }
...
function action_makecodeprivate()
{
    ...
    if (isset($_GET["rid"]) && !empty($_GET["rid"]) && is_numeric($_GET["rid"]))
        mysqli_query("UPDATE runs SET access='private' WHERE rid=" . $_GET["rid"]);
    else {
        $_SESSION["message"][] = "Run Data Updation Error : Insufficient Data";
        return;
    }
...
function action_makecodedisqualified()
{
    ...
    if (isset($_GET["rid"]) && !empty($_GET["rid"]) && is_numeric($_GET["rid"])) {
        mysqli_query("UPDATE runs SET result='DQ' WHERE rid=" . $_GET["rid"]);
...
function action_makecodedeleted()
{
    ...
    if (isset($_GET["rid"]) && !empty($_GET["rid"]) && is_numeric($_GET["rid"])) {
        mysqli_query("UPDATE runs SET access='deleted' WHERE rid=" . $_GET["rid"]);
