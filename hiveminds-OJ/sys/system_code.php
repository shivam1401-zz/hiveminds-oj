...
function display_code(){
	...
	if($error==4) echo "<tr><td colspan=10 style='padding:30;'>You are not authorized to access this code.</td></tr>";
		...
		if(($run["result"]!="RTE"||$_SESSION["status"]=="Admin") && !empty($run["error"])) echo "<tr><th colspan=10>Error Message</th></tr><tr><td colspan=10 style='text-align:left;padding:0;'><div class='limit'><pre class='brush:text'>".preg_replace("<br>","\n",filter($run["error"]))."</pre></div></td></tr>";
...
function action_submitcode(){
	...
	if($_SESSION["status"]!="Admin" && preg_match("^#[0-9]+ CQM\-[0-9]+$",$data["prgoup"]) && $admin["mode"]=="Passive"){ $_SESSION["message"][] = "Code Submission Error : You can no longer submit solutions to this problem."; return; }
...
function action_makecodepublic(){
	if($_SESSION["status"]!="Admin"){ $_SESSION["message"][] = "Access Denied : You need to be an Administrator to perform this action."; return; }
	if(isset($_GET["rid"]) && !empty($_GET["rid"]) && is_numeric($_GET["rid"])) mysqli_query("UPDATE runs SET access='public' WHERE rid=".$_GET["rid"]);
	else { $_SESSION["message"][] = "Run Data Updation Error : Insufficient Data"; return; }
	}
	
function action_makecodeprivate(){
	if($_SESSION["status"]!="Admin"){ $_SESSION["message"][] = "Access Denied : You need to be an Administrator to perform this action."; return; }
	if(isset($_GET["rid"]) && !empty($_GET["rid"]) && is_numeric($_GET["rid"])) mysqli_query("UPDATE runs SET access='private' WHERE rid=".$_GET["rid"]);
	else { $_SESSION["message"][] = "Run Data Updation Error : Insufficient Data"; return; }
	}

function action_makecodedisqualified(){
	if($_SESSION["status"]!="Admin"){ $_SESSION["message"][] = "Access Denied : You need to be an Administrator to perform this action."; return; }
	if(isset($_GET["rid"]) && !empty($_GET["rid"]) && is_numeric($_GET["rid"])){ mysqli_query("UPDATE runs SET result='DQ' WHERE rid=".$_GET["rid"]); $_SESSION["message"][] = "Code Disqualification Successful."; }
	else { $_SESSION["message"][] = "Run Data Updation Error : Insufficient Data"; return; }
	}
	
function action_makecodedeleted(){
	if($_SESSION["status"]!="Admin"){ $_SESSION["message"][] = "Access Denied : You need to be an Administrator to perform this action."; return; }
	if(isset($_GET["rid"]) && !empty($_GET["rid"]) && is_numeric($_GET["rid"])){ mysqli_query("UPDATE runs SET access='deleted' WHERE rid=".$_GET["rid"]); $_SESSION["message"][] = "Code Deletion Successful."; }
	else { $_SESSION["message"][] = "Run Data Updation Error : Insufficient Data"; return; }
	}
