...
function action_requestclar(){
	...
	if(preg_match($invalidchars,$_POST["query"])){ $_SESSION["message"][] = "Clarification Request Error : Invalid characters in Query."; return; }
...
function action_clarcache(){
	global $admin,$link;
	$team=array(); $data = mysqli_query($link,"SELECT tid,teamname FROM teams"); if(($data instanceof mysqli_result)) while($temp = mysqli_fetch_array($data)) $team[$temp["tid"]] = filter($temp["teamname"]);
	$prob=array(0=>"General"); $data = mysqli_query($link,"SELECT pid,name FROM problems WHERE status='Active'"); if(($data instanceof mysqli_result)) while($temp = mysqli_fetch_array($data)) $prob[$temp["pid"]] = filter($temp["name"]);
	...
	$filedata="";
	if(($data instanceof mysqli_result) && mysqli_num_rows($data)>0) while($temp = mysqli_fetch_array($data)){
...
function action_updateclar(){
	...
	if($_POST["field"]=="Reply"){
		...
		if(preg_match($invalidchars,$_POST["value"])){ $_SESSION["message"][] = "Clarification Update Error : Invalid characters in Reply."; return; }
		if(!empty($_POST["value"])) mysqli_query($link,"UPDATE clar SET time=".time().",reply='".addslashes(filter($_POST["value"]))."' WHERE time=".$_POST["time"]);
		else mysqli_query($link,"UPDATE clar SET reply='' WHERE time=".$_POST["time"]);
		}
	if($_POST["field"]=="Status"){
		if(!isset($_POST["time"]) || empty($_POST["time"]) || !isset($_POST["value"]) || empty($_POST["value"])){ $_SESSION["message"][] = "Clarification Update Error : Insufficient Data."; return; }
	...
	if($_POST["field"]=="Clear") mysqli_query($link,"UPDATE clar SET access='Delete'");
	action_clarcache();
	}
	
function display_clarifications(){
	global $admin,$invalidchars,$link;
	echo "<center>";
	...
	if($_SESSION["status"]=="Admin") $total = mysqli_query($link,"SELECT count(*) as total FROM clar WHERE ".($type=="public"?"access='Public'":($type=="private"?"access='Private'":"access!='Delete'"))." ".($reply=="no"?" AND reply='' ":($reply=="yes"?" AND reply!='' ":"")));
	else $total = mysqli_query($link,"SELECT count(*) as total FROM clar WHERE ".($type=="public"?"access='Public'":($type=="private"?"access='Private' AND tid=".$_SESSION["tid"]:"access!='Delete' AND (clar.access='Public' or tid=".$_SESSION["tid"].")"))." ".($reply=="no"?" AND reply='' ":($reply=="yes"?" AND reply!='' ":"")));
	...
	$team=array(); $data = mysqli_query($link,"SELECT tid,teamname,status FROM teams"); if(($data instanceof mysqli_result)) while($temp = mysqli_fetch_array($data)) $team[$temp["tid"]] = array("name"=>filter($temp["teamname"]),"status"=>$temp["status"]);
	$prob=array(0=>"General"); $data = mysqli_query($link,"SELECT pid,name FROM problems WHERE status='Active'"); if(($data instanceof mysqli_result)) while($temp = mysqli_fetch_array($data)) $prob[$temp["pid"]] = filter($temp["name"]);
	echo "<table width=100%><tr><th>Query / Response</th><th width='120px'>Options</th></tr>";
	if($_SESSION["status"]=="Admin") $data = mysqli_query($link,"SELECT * FROM clar WHERE ".($type=="public"?"access='Public'":($type=="private"?"access='Private'":"access!='Delete'"))." ".($reply=="no"?" AND reply='' ":($reply=="yes"?" AND reply!='' ":""))." ORDER BY time ASC LIMIT ".(($page-1)*$limit).",$limit");
	else $data = mysqli_query($link,"SELECT * FROM clar WHERE ".($type=="public"?"access='Public'":($type=="private"?"access='Private' AND tid=".$_SESSION["tid"]:"access!='Delete' AND (clar.access='Public' or tid=".$_SESSION["tid"].")"))." ".($reply=="no"?" AND reply='' ":($reply=="yes"?" AND reply!='' ":""))." ORDER BY time ASC LIMIT ".(($page-1)*$limit).",$limit");
	...
	if($_SESSION["tid"]){
		...
		if(($data instanceof mysqli_result)) while($problem = mysqli_fetch_array($data)) echo "<option value='".$problem["pid"]."'>".filter($problem["name"])."</option>";
		echo "</select></td></tr>";
