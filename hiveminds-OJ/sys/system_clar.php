...
function action_requestclar()
{
    ...
    if (!isset($_POST["query"]) || empty($_POST["query"]) || !isset($_POST["problem"])) {
...
function display_clarifications()
{
    ...
    if (isset($_GET["type"]) and in_array($_GET["type"], array(
        "public",
        "private"
    )))
        $type = $_GET["type"];
        ...
        $type = "all";
    if (isset($_GET["reply"]) and in_array($_GET["reply"], array(
        "yes",
        "no"
    )))
        $reply = $_GET["reply"];
