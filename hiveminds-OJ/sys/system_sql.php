...
function mysql_initiate(){
	...
	if(!mysqli_select_db($link,$mysql_database)){
		...
		if(!mysqli_select_db($link, $mysql_database)){ $_SESSION["message"][] = "SQL Error : Could Not Select Database."; return; }
		}
	$data = array_column(mysqli_fetch_all($link->query('SHOW TABLES')),0);
	//mysql_list_tables($mysql_database); 
	$table = array(); if(is_resource($data)) while($temp=mysqli_fetch_row($data)) $table[] = $temp[0];
	if(!in_array("teams",$table)){ mysqli_query($link,"CREATE TABLE teams (tid int not null primary key auto_increment,teamname tinytext,teamname2 tinytext,pass tinytext,status tinytext,score int,penalty bigint,name1 tinytext,roll1 tinytext,branch1 tinytext,email1 tinytext,phone1 tinytext,name2 tinytext,roll2 tinytext,branch2 tinytext,email2 tinytext,phone2 tinytext,name3 tinytext,roll3 tinytext,branch3 tinytext,email3 tinytext,phone3 tinytext,platform text,ip text,session tinytext,gid int not null)"); }
	if(!in_array("problems",$table)){ mysqli_query($link,"CREATE TABLE problems (pid int not null primary key auto_increment,code tinytext,name tinytext,type tinytext,status tinytext,pgroup tinytext,statement longtext,image blob,imgext tinytext,input longtext,output longtext,timelimit int,score int,languages tinytext,options tinytext)"); }
	if(!in_array("runs",$table)){ mysqli_query($link,"CREATE TABLE runs (rid int not null primary key auto_increment,pid int,tid int,language tinytext,name tinytext,code longtext,time tinytext,result tinytext,error text,access tinytext,submittime int,output longtext)"); }
	if(!in_array("admin",$table)){ mysqli_query($link,"CREATE TABLE admin (variable tinytext,value longtext)"); }
	if(!in_array("logs",$table)){ mysqli_query($link,"CREATE TABLE logs (time int not null primary key,ip tinytext,tid int,request tinytext)"); }
	if(!in_array("clar",$table)){ mysqli_query($link,"CREATE TABLE clar (time int not null primary key,tid int,pid int,query text,reply text,access tinytext,createtime int)"); }
	if(!in_array("groups",$table)){ mysqli_query($link,"CREATE TABLE groups (gid int not null primary key auto_increment, groupname tinytext, statusx int)"); }
...
function mysql_terminate(){
	...
	foreach($admin as $key=>$value){
		...
		if(($temp instanceof mysqli_result) && mysqli_num_rows($temp)>0) mysqli_query($link,"UPDATE admin SET value='".addslashes($value)."' WHERE variable='".addslashes($key)."'");
		else mysqli_query($link,"INSERT INTO admin VALUES ('".addslashes($key)."','".addslashes($value)."')");
		}
	$_SESSION["time"]=time();
	mysqli_close($link);
	}
...
function mysql_getdata($query){
	...
	if(!($t instanceof mysqli_result)) return NULL;
	$data = array();
	while($row = mysqli_fetch_array($t)) $data[] = $row;
	return $data;
	}
