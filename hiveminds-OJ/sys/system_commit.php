...
function action_commitdata(){
	...
	$temp = mysqli_query("SHOW TABLES LIKE '$tablename';");
	...
	$problems = mysqli_query("SELECT * FROM problems WHERE status='Active'"); if(!is_resource($problems)){ $_SESSION["message"][] = "Data Commitment Error : Failure 001."; return; }
	$nprob = mysql_num_rows($problems);
	$teams = mysqli_query("SELECT * FROM teams WHERE status='Normal'"); if(!is_resource($teams)){ $_SESSION["message"][] = "Data Commitment Error : Failure 002."; return; }
	...
	mysqli_query("INSERT INTO $tablename (info,id,name,score) VALUES ('system-fields','Timestamp','Commit Name','Status');");
	mysqli_query("INSERT INTO $tablename (info,id,name,score) VALUES ('system','".time()."','".addslashes($_POST["recordname"])."','Active');");
	
	// Insert Problem Index
	mysqli_query("INSERT INTO $tablename (info,id,name,score,data_1_1,data_1_2,data_1_3,data_1_4,data_1_5,data_1_6) VALUES ('problem-fields','Problem ID','Problem Name','Problem Score','Problem Statement','Problem Input','Problem Output','Problem Time Limit','Problem Type','Problem Statistics');");
	
	// Insert Problems
	$pids = array();
	while($problem = mysql_fetch_array($problems)){
		$stat1 = mysqli_query("SELECT count( DISTINCT tid ) FROM runs WHERE result='AC' AND pid=".$problem["pid"]);
			if(!is_resource($stat1) || !mysql_num_rows($stat1)) echo mysql_error();//continue;
			$stat1 = mysql_fetch_array($stat1);
		$stat2 = mysqli_query("SELECT count( DISTINCT tid ) FROM runs WHERE pid=".$problem["pid"]);
	...
	while($team = mysql_fetch_array($teams)){
		...
		foreach($pids as $pid){
			$ac3 = mysqli_query("SELECT * FROM runs WHERE tid='$team[tid]' AND pid='$pid' AND result='AC' ORDER BY rid DESC LIMIT 0,1"); if(is_resource($ac3) && mysql_num_rows($ac3)==1) $ac3 = mysql_fetch_array($ac3); else $ac3 = NULL;
			if($ac3!=NULL){
				$ac2 = mysqli_query("SELECT * FROM runs WHERE tid='$team[tid]' AND pid='$pid' AND result='AC' AND access!='deleted' ORDER BY rid ASC  LIMIT 0,1"); if(is_resource($ac2) && mysql_num_rows($ac2)==1) $ac2 = mysql_fetch_array($ac2); else $ac2 = NULL;
				$ac1 = mysqli_query("SELECT count(*) FROM runs WHERE tid='$team[tid]' AND pid='$pid' AND result!='AC' AND access!='deleted' AND rid<=".$ac2["rid"])+1;	if(is_resource($ac1) && mysql_num_rows($ac1)==1) $ac1 = mysql_fetch_array($ac1); else $ac1 = NULL;
				$query.=",'$ac1[0]','$ac2[language]','$ac2[code]','$ac2[result]','$ac2[time]','$ac2[error]'";
				}
			else {
				$ac1 = mysqli_query("SELECT count(*) FROM runs WHERE tid='$team[tid]' AND pid='$pid' AND result!='AC' AND access!='deleted'"); 	if(is_resource($ac1) && mysql_num_rows($ac1)==1) $ac1 = mysql_fetch_array($ac1); else $ac1 = NULL;
				$ac0 = mysqli_query("SELECT * FROM runs WHERE tid='$team[tid]' AND pid='$pid' AND access!='deleted' ORDER BY rid DESC LIMIT 0,1"); if(is_resource($ac0) && mysql_num_rows($ac0)==1) $ac0 = mysql_fetch_array($ac0); else $ac0 = NULL;
				if($ac0==NULL) $query.=",'0','','','','',''";
...
function action_commitupdate(){
	...
	$_POST["recordname"] = preg_replace("\"","'",$_POST["recordname"]);
	mysqli_query("UPDATE ".$_POST["tablename"]." SET name='".addslashes($_POST["recordname"])."' WHERE info='system'");
	//echo "UPDATE ".$_POST["tablename"]." SET name='".addslashes($_POST["recordname"])."' WHERE info='system'";
	if($_POST["status"]=="Delete") mysqli_query("DROP TABLE ".$_POST["tablename"]);
	else if($_POST["status"]=="Inactive") mysqli_query("UPDATE ".$_POST["tablename"]." SET score='Inactive' WHERE info='system'");
	else if($_POST["status"]=="Active") mysqli_query("UPDATE ".$_POST["tablename"]." SET score='Active' WHERE info='system'");
...
function action_scoreboard(){
	...
	$data = mysqli_query("SELECT tid,teamname FROM teams WHERE status='Normal'");
	...
	foreach($tables as $table) if(preg_match("^backup_",$table)){
		// mysqli_query("INSERT INTO $tablename (info,id,name,score) VALUES ('system-fields','Timestamp','Commit Name','Status');");
		$system = mysqli_query("SELECT * FROM $table WHERE info='system' and score='Active'");
		if(!is_resource($system) || mysql_num_rows($system)==0) continue;
		$system = mysql_fetch_array($system);
		$names[] = $system["name"];
		foreach($tids as $tid){
			$data = mysqli_query("SELECT score FROM $table WHERE info='team' AND id='$tid'");
	...
	$filedata.="</table>";
	$temp = mysqli_query("SELECT * FROM admin WHERE variable='scoreboard'");
	if(is_resource($temp) && mysql_num_rows($temp)>0) mysqli_query("UPDATE admin SET value='".addslashes($filedata)."' WHERE variable='scoreboard'");
		else mysqli_query("INSERT INTO admin VALUES ('scoreboard','".addslashes($filedata)."')");
	}
