...
function action_commitdata()
{
    ...
    $problems = mysqli_query("SELECT * FROM problems WHERE status='Active'");
    if (!is_resource($problems)) {
        $_SESSION["message"][] = "Data Commitment Error : Failure 001.";
        return;
    }
    $nprob = mysql_num_rows($problems);
    $teams = mysqli_query("SELECT * FROM teams WHERE status='Normal'");
    ...
    while ($team = mysql_fetch_array($teams)) {
        ...
        foreach ($pids as $pid) {
            $ac3 = mysqli_query("SELECT * FROM runs WHERE tid='$team[tid]' AND pid='$pid' AND result='AC' ORDER BY rid DESC LIMIT 0,1");
            if (is_resource($ac3) && mysql_num_rows($ac3) == 1)
                $ac3 = mysql_fetch_array($ac3);
            else
                $ac3 = NULL;
            if ($ac3 != NULL) {
                $ac2 = mysqli_query("SELECT * FROM runs WHERE tid='$team[tid]' AND pid='$pid' AND result='AC' AND access!='deleted' ORDER BY rid ASC  LIMIT 0,1");
                if (is_resource($ac2) && mysql_num_rows($ac2) == 1)
                    $ac2 = mysql_fetch_array($ac2);
                else
                    $ac2 = NULL;
                $ac1 = mysqli_query("SELECT count(*) FROM runs WHERE tid='$team[tid]' AND pid='$pid' AND result!='AC' AND access!='deleted' AND rid<=" . $ac2["rid"]) + 1;
                ...
                $ac1 = mysqli_query("SELECT count(*) FROM runs WHERE tid='$team[tid]' AND pid='$pid' AND result!='AC' AND access!='deleted'");
                if (is_resource($ac1) && mysql_num_rows($ac1) == 1)
                    $ac1 = mysql_fetch_array($ac1);
                else
                    $ac1 = NULL;
                $ac0 = mysqli_query("SELECT * FROM runs WHERE tid='$team[tid]' AND pid='$pid' AND access!='deleted' ORDER BY rid DESC LIMIT 0,1");
...
function action_commitupdate()
{
    ...
    if ($_POST["status"] == "Delete")
        mysqli_query("DROP TABLE " . $_POST["tablename"]);
    else if ($_POST["status"] == "Inactive")
        mysqli_query("UPDATE " . $_POST["tablename"] . " SET score='Inactive' WHERE info='system'");
    else if ($_POST["status"] == "Active")
        mysqli_query("UPDATE " . $_POST["tablename"] . " SET score='Active' WHERE info='system'");
...
function action_scoreboard()
{
    ...
    foreach ($tables as $table)
        if (preg_match("^backup_", $table)) {
            // mysqli_query("INSERT INTO $tablename (info,id,name,score) VALUES ('system-fields','Timestamp','Commit Name','Status');");
            $system = mysqli_query("SELECT * FROM $table WHERE info='system' and score='Active'");
            if (!is_resource($system) || mysql_num_rows($system) == 0)
                continue;
    ...
    if (is_resource($temp) && mysql_num_rows($temp) > 0)
        mysqli_query("UPDATE admin SET value='" . addslashes($filedata) . "' WHERE variable='scoreboard'");
    else
        mysqli_query("INSERT INTO admin VALUES ('scoreboard','" . addslashes($filedata) . "')");
}
