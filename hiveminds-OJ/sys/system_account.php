...
function action_register()
{
    ...
    foreach ($_POST as $key => $value)
        if (preg_match("/^reg_/i", $key) && !preg_match("[23]$", $key) && empty($_POST[$key])) {
            $_SESSION["message"][] = "Registeration Error : Insufficient Data";
            return;
        }
    ...
    foreach ($_POST as $key => $value)
        if (preg_match("/^reg_/i", $key) && !preg_match("^reg_pass", $key) && preg_match($invalidchars, $value)) {
            $_SESSION["message"][] = "Registeration Error : Value of $key contains invalid characters.";
            return;
        }
    foreach ($_POST as $key => $value)
        if (preg_match("/^reg_/i", $key) && !preg_match("^reg_pass", $key) && strlen($value) > 30) {
            $_SESSION["message"][] = "Registeration Error : Value of $key too long.";
            return;
        }
    ...
    $temp1             = $temp2 = array();
    $_POST["reg_ip"]   = addslashes(json_encode(array(
        $_SERVER["REMOTE_ADDR"]
    )));
    foreach ($_POST as $key => $value)
        if ($key != "reg_pass1" && $key != "reg_pass2") {
            $temp1[] = preg_replace("reg_", "", $key);
            if ($key == "reg_ip")
                $temp2[] = $value;
            else
                $temp2[] = filter($value);
    ...
    if ($auto)
        mysqli_query($link, "INSERT INTO teams (" . implode($temp1, ",") . ",status,score) VALUES (\"" . implode($temp2, "\",\"") . "\",\"Normal\",0)");
    else
        mysqli_query($link, "INSERT INTO teams (" . implode($temp1, ",") . ",status,score) VALUES (\"" . implode($temp2, "\",\"") . "\",\"Waiting\",0)");
    $_SESSION["message"][] = "Registeration Successful";
    if (!$auto)
        $_SESSION["message"][] = "You may begin to use this account once it has been authorized by an Administrator.";
}
...
function action_updateteam()
{
    ...
    if (!isset($_POST["update_tid"]) || empty($_POST["update_tid"])) {
        $_SESSION["message"][] = "Team Data Updation Error : Insufficient Data";
        return;
    }
    // NA : Password Verification
    foreach ($_POST as $key => $value)
        if (preg_match("/^update_/i", $key) && !preg_match("^update_pass", $key) && preg_match($invalidchars, $value)) {
            $_SESSION["message"][] = "Team Data Updation Error : Value of $key contains invalid characters.";
            return;
        }
    foreach ($_POST as $key => $value)
        if (preg_match("/^update_/i", $key) && !preg_match("^update_pass", $key) && strlen($value) > 30) {
...
function action_login()
{
    ...
    if ($admin["mode"] == "Lockdown" && $t["status"] != "Admin" && !$_SESSION["ghost"]) {
        ...
        return;
    } // Unauthorized Login During Lockdown
