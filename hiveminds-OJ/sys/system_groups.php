...
function action_group_create(){
	...
	if(preg_match("[^A-Za-z0-9\.\_\-]",$_POST["groupname"])){ $_SESSION["message"][] = "Group Error : Invalid characters in Group Name."; return; }
	$data = mysqli_query($link,"SELECT * FROM groups WHERE statusx<3 AND groupname='".mysqli_real_escape_string($link,$_POST["groupname"])."';");
	if(!($data instanceof mysqli_result) or mysqli_num_rows($data)>0){ $_SESSION["message"][] = "Group Error : This Group Name has already been taken."; return; }
...
function action_group_modify(){
	...
	if(preg_match("[^A-Za-z0-9\.\_\-]",$_GET["groupname"])){ $_SESSION["message"][] = "Group Error : Invalid characters in Group Name."; return; }
	$data = mysqli_query($link,"SELECT * FROM groups WHERE statusx<3 AND groupname='".mysqli_real_escape_string($link,$_POST["groupname"])."';");
	if(!($data instanceof mysqli_result) or mysqli_num_rows($data)>0){ $_SESSION["message"][] = "Group Error : This Group Name has already been taken."; return; }
...
function action_group_status(){
	...
	if(!($data instanceof mysqli_result) or mysqli_num_rows($data)==0){ $_SESSION["message"][] = "Group Error : Could not select Group."; return; }
	mysqli_query($link,"UPDATE groups SET statusx=$_GET[status] WHERE gid=$_GET[gid];");
	// if($_GET["status"]=="3") mysqli_query($link,"UPDATE teams SET gid=0 WHERE gid=$_GET[gid];"); // delete
